...
public class GameDAO implements IGameDAO {
    ...
    public Game selectByName(SQLiteDatabase db, String str){
        String[] selectionArgs = {str};
        Cursor cursor = db.query(
                this.tableName,         // The table to query
                this.projection,        // The array of columns to return (pass null to get all)
                Game.NAME_COLUMN + " LIKE ?",              // The columns for the WHERE clause
                selectionArgs,          // The values for the WHERE clause
                null,          // don't group the rows
                null,           // don't filter by row groups
                null               // The sort order
    ...
    public boolean delete(SQLiteDatabase db, Game game) {
        return db.delete(Game.TABLE_NAME, Game.ID_COLUMN + "=" + game.getId(), null) > 0;
    }
}
